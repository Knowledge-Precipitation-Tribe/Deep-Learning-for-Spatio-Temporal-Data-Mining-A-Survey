...
3. **若数学公式显示出现问题大家也可通过jupyter notebook链接查看：[nbviewer](https://nbviewer.jupyter.org/github/Knowledge-Precipitation-Tribe/Deep-Learning-for-Spatio-Temporal-Data-Mining-A-Survey/blob/master/Deep-Learning-for-Spatio-Temporal-Data-Mining.ipynb)**
...
## [Attention Mechanism](#content)

注意力机制，旨在提高机器翻译上的Encoder-Decoder RNN的性能[5]。Encoder-Decoder RNN的一个主要限制是它将输入序列编码为固定长度的内部表示形式，这对于长输入序列会导致较差的性能。为了解决这个问题，注意力可以使模型学习在预测目标序列中每个单词的过程中要注意源序列中哪些编码的单词以及要注意的程度。尽管最初是在机器翻译中以单词序列数据作为输入来提出注意力的，但实际上它可以应用于任何种类的输入，例如图像，这被称为视觉注意。由于许多ST数据可以表示为顺序数据（时间序列和轨迹）和类似图像的空间图，因此也可以将注意力引入深度学习模型中，以提高各种STDM任务的性能[19]，[38]，[39]，[57]，[81]，[88]，[98]，[142]，[198]。

STDM中使用的神经注意机制通常可分为空间域注意[19]，[39]和时域注意[38]，[57]，[81]，[198]。一些作品同时使用时空领域的注意力[88]，[98]，[142]。[39]提出了一个在空间领域的组合注意力模型。它同时利用了“软注意力”和“硬连线”注意力，以便将轨迹信息从本地社区映射到感兴趣的行人的未来位置。[57]提出了一种称为DeepCrime的细心分层递归网络模型，用于犯罪预测。时域注意机制被应用于捕获从先前时隙学习的犯罪模式的相关性，以帮助预测未来的犯罪发生，并自动将重要性权重分配给在不同时间帧学习的隐藏状态。在提出的注意力机制中，通过softmax函数推导归一化的重要性权重来估算过去时隙中犯罪发生的重要性。[88]提出了一个多层次的注意力网络，用于预测由部署在不同地理空间位置的传感器产生的地理感测时间序列，以连续和协同地监测周围环境，例如空气质量。具体而言，在第一级注意力中，提出了一种由局部空间注意力和全局空间注意力组成的空间注意力机制，以捕获不同传感器时间序列之间的复杂空间相关性。在第二级注意中，应用时间注意来对时间序列中不同时间间隔之间的动态时间相关性进行建模。

# [Application](#content)

大量的ST数据来自各种应用领域，例如交通，按需服务，气候和天气，人员流动，基于位置的社交网络（LBSN），犯罪分析和神经科学。表二显示了上述应用领域的相关工作。可以看到，由于城市交通数据和人员流动数据的可用性越来越高，工程中最大的一部分属于交通和人员流动。在本节中，我们将描述用于STDM的深度学习技术在不同应用程序中的应用。

![table2](./img/table2.png)

## [Transportation](#content)

随着从各种传感器（例如环路检测器，道路摄像机和GPS）收集的交通数据的可用性不断提高，迫切需要利用深度学习方法来学习交通数据之间的复杂且高度非线性的时空相关性，以促进 各种任务，例如交通流量预测[30]，[60]，[90]，[121]，[136]，[167]，交通事件检测[125]，[189]，[199]和交通拥堵预测[108]，[137]。这样的与运输有关的ST数据通常包含交通速度，交通量或交通事故，区域的路段的位置以及时间的信息。在不同的应用场景中，可以将运输数据建模为时间序列，空间地图和ST栅格。
例如，在道路网络规模的交通流量预测中，可以将从多个路环传感器收集的交通流量数据建模为栅格矩阵，其中一维是传感器的位置，另一维是时隙[106]。还可以基于部署传感器的道路链接之间的连接，将回路传感器连接为传感器图，并且可以将路网的交通数据建模为空间图，从而可以应用GraphCNN模型[ 85]，[175]。在道路级交通预测中，每条道路的历史交通流量数据均按时间序列建模，然后将RNN或其他深度学习模型用于单条道路的交通预测[60]，[90]，[ 167]。

## [On-Demand Service](#content)

近年来，由于移动电话的广泛使用，诸如Uber，Mobike，DiDi，GoGoVan之类的各种按需服务已变得越来越流行。按需服务已通过为人们提供他们想要的东西和地点而接管了传统业务。许多按需服务会产生大量的ST数据，这些数据涉及客户的位置和所需的服务时间。例如，Uber和DiDi分别是美国和中国的两个受欢迎的按需乘车点播服务提供商。它们都通过智能手机应用程序向用户提供包括出租车叫车，私家车叫车和社交共享的服务。为了更好地满足客户的需求并改善服务，一个关键问题是如何准确预测不同位置和时间的服务需求和供应。STDM的深度学习方法在按需服务中的应用主要集中在预测需求和供应。[1]提出应用深度学习方法来预测无底座自行车共享系统的需求-供应分布。[92]提出了一个CNN图模型来预测大型自行车共享网络中车站级别的小时需求。[126]，[174]提出使用LSTM模型来预测不同地区的出租车需求。[146]应用ResNet模型来预测在线汽车叫车服务的供应需求。通常将被研究城市不同区域的历史需求供应建模为空间地图或栅格张量，以便将CNN，RNN和组合模型用于预测未来。

## [Climate & Weather](#content)

气候科学是对气候的科学研究，科学定义为一段时间内的平均天气状况。天气数据通常包含大气和海洋条件（例如温度，压力，风量和湿度），这些条件是由部署在固定或浮动位置的各种气候传感器收集的。由于不同地点的气候数据通常表现出较高的时空相关性，因此STDM技术被广泛用于短期和长期天气预报。特别是，随着深度学习技术的最新发展，许多工作试图结合深度学习模型来分析各种天气和环境数据[79]，[129]，例如空气质量推断[19]，[94]，降水预测[100]，[161]，风速预测[96]，[200]和极端天气检测[100]。与气候和天气有关的数据可以是空间图（例如雷达反射率图像）[188]，时间序列（例如风速）[17]和事件（例如极端天气事件）[100]。[19]提出了一种神经注意模型来预测不同监测站的城市空气质量数据。[100]提出使用CNN模型来检测气候数据库中的极端天气。CNN模型也可以用于从遥感图像中估计降水[100]。
...
执法机构存储了许多城市中报告的犯罪信息，并公开了犯罪数据以供研究之用。犯罪事件数据通常具有犯罪类型（例如纵火，殴打，入室盗窃，抢劫，盗窃和故意破坏），以及犯罪发生的时间和地点。可以使用该数据研究犯罪的模式以及执法政策对某个地区犯罪数量的影响，目的是减少犯罪[4]。由于犯罪发生在城市的不同区域，通常表现出高度的时空相关性，因此可以将深度学习模型与城市的犯罪账户热图结合使用，以获取这种复杂的相关性[31]，[57]，[145]。例如，[31]提出了一个基于CNN的时空犯罪网络，以预测第二天市区每个区域的犯罪风险。[145]提出利用ST-ResNet模型来共同预测洛杉矶地区的犯罪分布。[57]开发了一个新的犯罪预测框架– DeepCrime，它是一种深度神经网络体系结构，可发现动态犯罪模式并探索犯罪与城市空间中其他普遍存在的数据之间的相互依存关系。如前所述，犯罪数据是典型的ST事件数据，但通常通过在时空范围内合并数据将其表示为空间地图，从而可以将深度学习模型应用于分析。

## [Neuroscience](#content)

近年来，脑成像技术已成为神经科学领域的热门话题。此类技术包括功能性磁共振成像（fMRI），脑电图（EEG），脑磁图（MEG）和功能性近红外光谱（fNIRS）。这些技术所测量的神经活动的时空分辨率与另一种技术有很大不同。fMRI从数百万个位置测量神经活动，而仅从数十个位置测量EEG数据的神经活动。fMRI通常每两秒钟测量一次活动，而EEG数据的时间分辨率通常为1毫秒。由于其空间分辨能力，fMRI和EEG结合深度学习方法已被广泛用于神经科学研究[34]，[63]，[113]，[128]。正如我们之前讨论的那样，深度学习模型主要通过使用fMRI数据或EEG数据（例如疾病分类[34]，脑功能网络分类[113]和大脑激活分类[63]）在神经科学中用于分类任务。例如，长期记忆网络（LSTM）用于识别自闭症谱系障碍（ASD）[34]，卷积神经网络（CNN）用于诊断轻度轻度认知障碍（aMCI）[113]和前馈 神经网络（FNN）被用于对精神分裂症进行分类[119]。

# [Open Problems](#content)

尽管已经提出了许多深度学习方法并将其广泛用于上述各种应用领域中的STDM，但是由于ST数据的高度复杂，大量和快速增长，仍然存在挑战。在本节中，我们提供了一些尚待解决的悬而未决的问题，这些问题目前尚待解决，将来需要进一步研究。

## [Interpretable models](#content)

当前针对STDM的深度学习模型通常被认为是缺乏解释性的黑匣子。可解释性使深度学习模型能够以易于理解的方式向人类解释或呈现模型行为，并且它是机器学习模型必不可少的部分，以更好地为人们服务并为社会带来利益[29]。考虑到复杂的数据类型和ST数据的表示形式，与其他类型的数据（例如图像和单词令牌）相比，设计可解释的深度学习模型更具挑战性。尽管先前的工作中使用注意力机制来提高模型的可解释性，例如周期性和局部空间依赖性[19]，[57]，[88]，但如何为STDM任务构建更可解释的深度学习模型仍未得到很好的研究。并且仍然是一个未解决的问题。

## [Deep learning model selection](#content)

对于给定的STDM任务，有时可以收集多种类型的相关ST数据，并且可以选择不同的数据表示形式。如何正确选择ST数据表示形式和相应的深度学习模式尚未得到很好的研究。例如，在交通流量预测中，一些工作将每个道路的交通流量数据建模为一个时间序列，以便使用RNN，DNN或SAE进行预测[104]，[136]；一些工作将多条道路的交通流量数据建模为空间地图，以便将CNN应用于预测[184]；一些工作将道路网络的交通流量数据建模为图形，从而采用GraphCNN [85]。缺乏关于如何正确选择ST数据的深度学习模型和数据表示以更好地解决所研究STDM任务的深入研究。

## [Broader applications to more STDM tasks](#content)

尽管深度学习模型已在上述各种STDM任务中广泛使用，但仍有一些深度学习模型尚未解决的任务，例如频繁模式挖掘和关系挖掘[4]，[87]。深度学习的主要优点是其强大的特征学习能力，这对于某些STDM任务（例如很大程度上依赖于高质量特征的预测性学习和分类）必不可少。但是，对于某些STDM任务（例如频繁模式挖掘和关系挖掘），学习高质量特征可能没有帮助，因为这些任务不需要特征。根据我们的评论，目前很少有甚至没有作品将深度学习模型用于上述任务。因此，如何将深度学习模型与深度学习模型或诸如频繁模式挖掘和图形模型之类的传统模型集成在一起，可以扩展到更多STDM任务的广泛应用仍然是一个悬而未决的问题。

## [Fusing multi-modal ST datasets](#content)

在大数据时代，多模态ST数据集在神经成像，气候科学和城市交通等许多领域都越来越可用。例如，在神经成像中，fMRI和DTI都可以使用提供不同时空分辨率的不同技术来捕获大脑活动的成像数据[61]。很少研究如何使用深度学习模型将它们有效地融合在一起，以更好地执行疾病分类和脑活动识别的任务。包括出租车轨迹数据，共享自行车出行数据和城市公共交通出入库数据在内的多式联运数据都可以从不同角度反映城市人群的流动性[30]。将它们融合在一起而不是分别进行分析，可以更全面地捕获基础移动模式并做出更准确的预测。尽管最近有尝试将深度学习模型用于从不同城市之间的人群流数据中转移知识的尝试[151]，[172]，但如何将多模式ST数据集与深度学习模型融合仍未得到很好的研究，需要解决。未来会有更多的研究关注。

# [Conclusion](#content)

在本文中，我们对探索STDM深度学习技术的最新进展进行了全面概述。我们首先对ST数据的不同数据类型和表示进行分类，并简要介绍STDM常用的深度学习模型。对于不同类型的ST数据及其表示，我们展示了适合处理它们的相应深度学习模型。然后，我们给出一个通用框架，该框架展示了利用深度学习模型解决STDM任务的管道。在此框架下，我们根据ST数据类型的分类和STDM任务（包括预测性学习，表示性学习，分类，估计和推断，异常检测等）概述当前的工作。接下来，我们总结了STDM深度学习技术在不同领域中的应用，包括交通，按需服务，气候和天气，人员流动，基于位置的社交网络（LBSN），犯罪分析和神经科学。最后，我们列出了一些未解决的问题，并指出了这项快速发展的研究的未来研究方向
